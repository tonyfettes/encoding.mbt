...
static inline bool
tonyfettes_encoding_decode_utf8_to_char_array__is_ascii(
...
static inline void
tonyfettes_encoding_decode_utf8_to_char_array__ascii(
  const uint8_t *restrict sptr,
...
tonyfettes_encoding_decode_utf8_to_char_array(
  ...
  while (1) {
    ...
    if (slen >= DECODE_SIMD_SIZE && (uintptr_t)sptr % 8 == 0) {
      if (tonyfettes_encoding_decode_utf8_to_char_array__is_ascii(sptr)) {
        tonyfettes_encoding_decode_utf8_to_char_array__ascii(sptr, tptr);
...
tonyfettes_encoding_validate_utf8(
  moonbit_bytes_t s,
  int32_t soff,
  int32_t slen
) {
...
tonyfettes_encoding_unsafe_decode_utf8_to_char_array(
  moonbit_bytes_t s,
  int32_t soff,
  int32_t slen,
  struct moonbit_int32_array *restrict t
...
enum { ENCODE_SIMD_SIZE = 16 };

static inline bool
tonyfettes_encoding_encode_char_array_to_utf8__is_ascii(
  const int32_t *restrict sptr
) {
  int32_t mask = 0;
  for (int32_t i = 0; i < ENCODE_SIMD_SIZE; i++) {
    mask |= sptr[i];
  }
  return mask < 0x80;
}
...
tonyfettes_encoding_encode_char_array_to_utf8__ascii(
  const int32_t *restrict sptr,
...
MOONBIT_FFI_EXPORT
int32_t
tonyfettes_encoding_encode_char_array_to_utf8(
  ...
  for (size_t i = 0; i < slen;) {
    if (i + ENCODE_SIMD_SIZE < slen && i % 8 == 0) {
      if (tonyfettes_encoding_encode_char_array_to_utf8__is_ascii(
            sptr + i
          )) {
        tptr = tonyfettes_encoding_encode_char_array_to_utf8__ascii(
          sptr + i, tptr
        );
        i += ENCODE_SIMD_SIZE;
        continue;
